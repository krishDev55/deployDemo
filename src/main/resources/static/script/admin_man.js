...
const defaultState = { products1: [], customers: [], products: [] }
...
async function renderInventory(filter = '', ty = '') {

    // let products = JSON.parse(localStorage.getItem('products') || '[]');
    let products = JSON.parse(localStorage.getItem('products1') || '[]');

    let getProductByFileAPI = `${host_Api}/app/v1/product/fileHandle`;
    const responce = new Request(getProductByFileAPI, {
        method: "get",
    });
    ...
    if (filter === "inventory") {
        ...
        if (ty === "") {
            ...
            rows = products.filter(p => p.name.toLowerCase().includes(ty) || (String(p.sku).includes(ty)))
        }
        invTable.innerHTML = rows.map((p, i) => {
            let atrValue = "";
            for (const atr of p.attributes) {
                if (atr.id == 80) {
                    atrValue = atr.value;
                }
            }
            ...
            return `<tr class="${p.qty < 5 ? 'qty-low' : ''}">
                        <td>${p.sku}</td>
                        <td>${p.name}</td>
                        <td>${p.category || '-'}</td>
                        <td>${atrValue || '-'}</td>
        ...
        const f = filter.toLowerCase();
        const rows = products
            //.filter(p => p.name.toLowerCase().includes(f) || (String(p.sku).includes(f)))
...
async function sort(sortBy) {
    ...
    let rows;
    if (sortBy === "asd") {
        rows = products.sort((a, b) => a.qty - b.qty);
        document.getElementById("sortQty").setAttribute("onclick", "sort('desc')");
        viewName = "asd";
        renderInventory(viewName)
    }
    else {
        rows = products.sort((a, b) => b.qty - a.qty);
        document.getElementById("sortQty").setAttribute("onclick", "sort('asd')");
        viewName = "desc";
        renderInventory(viewName)
    }
    /*   // const rows = products;
       ...
       document.getElementById('inv-count').textContent = products.length
       document.getElementById('stat-count').textContent = state.products.length*/
...
document.getElementById('add-by-sku').addEventListener('click', () => {
    searchProduct("billing");
})

// add product to cart by search  Basic Live Search (on keyboard typing)
document.getElementById("search-product").addEventListener("input", function () {
    searchProduct("billing");

})

//  search  Inventory Product Live Search (on keyboard typing)
document.getElementById("inv-search").addEventListener("input", function () {
    searchProduct("inventory");
})

function searchProduct(ty) {
    let q = null;
    if (ty === "billing") { q = document.getElementById('search-product').value.trim().toLowerCase() }
    else if (ty === "inventory") { q = document.getElementById('inv-search').value.trim().toLowerCase() }

    // if (!q) return alert('Enter product name or SKU')
    let found = JSON.parse(localStorage.getItem("products1"))
        .filter(p => p.name.toLowerCase() === q || p.name.toLowerCase().includes(q))
    // console.log(found)
    //.find(p => p.sku.toLowerCase() === q || p.name.toLowerCase().includes(q));
        ...
        if (ty === "billing") {
            ...
            for (const e of found) {
                ...
                inponC = inponC + `
                        ...
                        <div>
                            <h5> ${e.sku} </h5>
                            <button onclick="cl123('${e.sku}')">  click </button>
                        </div>
                        </div>
            `
            }
            popupContent2.innerHTML = inponC;
        }
        else if (ty === "inventory") {

            renderInventory(ty, q);
        }
...
async function cl123(elem) {
    // let item = JSON.parse(localStorage.getItem("products1"));
    // let cart = JSON.parse(localStorage.getItem('cart') || '[]');
    localStorage.getItem("cart")
    let apia = `${host_Api}/app/v1/product/Sku/${elem}`
    const responce = new Request(apia, {
        method: "get",
    });
    const res = await fetch(responce);
    // const data = await res.json();
    res.json().then((data) => {
        const item = { sku: data.id, name: data.product_name, price: data.price, qty: 1 };
...
function generateInvoiceHTML() {
    ...
    let saveBillsToBack = `${host_Api}/app/v1/savebill`;
...
document.getElementById('makeQr').addEventListener('click', () => {
    window.location.href = `${host_Api}/product`;
...
const now = new Date();
const sample = [
    /* { order_id: 'ORD-1001', ids: 'Rahul Sharma', date: iso(new Date(now.getFullYear(), now.getMonth(), now.getDate())), items: 3, totalPprice: 1299.00, status: 'Paid' }*/
...
function getAllOrders() {
    ...
    let urlApi = `${host_Api}/app/v1/AllOrders`;
        ...
        .then((responce) => {
            ...
            j.then((data) => {
                ...
                for (const e of data) {
                    ...
                    let qnty = 0;
                    for (const ordL of e.ordList) {
                        qnty = qnty + ordL.qty;
                    }

                    e.date = iso(new Date(Number(e.date)));
                    e.items = qnty;
                    e.amount = e.totalPprice;

                    sample.push(e)


                }
                localStorage.setItem(ordersKey, JSON.stringify(sample));
...
function filterByRange(range) {
    const today = new Date(); let from = null, to = null; switch (range) {
    ...
    return orders.filter(o => {
        ...
        if (from && to) return od >= from && od <= to;
        return true;
    });
}
...
function renderTable(list) {
    ...
    tbody.innerHTML = list.map(o => {
        ...
        return `<tr>						
                                    ...
                                    <td>${o.ids}</td><td>${o.items}</td>
                                    <td>₹${Number(o.totalPprice).toFixed(2)}</td>
                                    <td>${o.status}</td>
                             </tr>`
    }
...
document.getElementById('searchBtn').addEventListener('click', () => {
    const q = document.getElementById('searchInput').value.trim().toLowerCase(); const sd = document.getElementById('searchDate').value;
    let list = orders.slice(); if (q) { list = list.filter(o => o.id.toLowerCase().includes(q) || o.customer.toLowerCase().includes(q)); }
    if (sd) {
        const dateFrom = new Date(sd + 'T00:00:00'); const dateTo = new Date(sd + 'T23:59:59.999');
        list = list.filter(o => { const od = new Date(o.date); return od >= dateFrom && od <= dateTo; });
    }
...
function timeAgo(timestamp) {
    ...
    if (sec < 60) {
        return { text: `${sec} sec ago`, level: "normal", numt: `${sec}` };
    }

    const min = Math.floor(sec / 60);
    if (min < 3) {
        return { text: `${min} min ago`, level: "warning", numt: `${min}` };
    }

    if (min >= 3) {
        return { text: `${min} min ago`, level: "danger", numt: `${min}` };
    }

    const hr = Math.floor(min / 60);
    if (hr < 24) {
        return { text: `${hr} hours ago`, level: "danger", numt: `${hr}` };
    }

    const day = Math.floor(hr / 24);
    return { text: `${day} days ago`, level: "danger", numt: `${day}` };
}
...
let viewsA = (ord_id) => {
    let OrdUrl = `${host_Api}/app/v1/get_ordList/${ord_id}`;
    ...
    fetch(responce).then((data) => {
        ...
        d.then((value) => {
            ...
            let content = "";
            document.getElementById("pickerId").innerHTML = value.pickerId;
            document.getElementById("pickerName").innerHTML = value.pickerName;



            // document.getElementById("tms").innerHTML = timeAgo(value.time).text;

            value.ordlist.forEach(e => {
                content = content + `
                                                        <Div class="inPonC">
                                                                <div>
